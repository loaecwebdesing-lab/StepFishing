...
(function () {
    ...
    const LEGACY_ORNAMENT_IDS = new Set(['ornement1']);
    ...
    function normalizeOwned(list) {
        ...
        return owned.filter(id => catalogById[id] && !LEGACY_ORNAMENT_IDS.has(id));
    }

    function sanitizeEquippedCosmetic(id) {
        if (LEGACY_ORNAMENT_IDS.has(id) || !catalogById[id]) return 'default';
        return id;
    }
    ...
    function loadFromSave(data) {
        ...
        s.equippedCosmetic = sanitizeEquippedCosmetic(data?.equippedCosmetic);
    ...
    function getEquippedId() {
        const s = getState();
        return sanitizeEquippedCosmetic(s?.equippedCosmetic || 'default');
    }
